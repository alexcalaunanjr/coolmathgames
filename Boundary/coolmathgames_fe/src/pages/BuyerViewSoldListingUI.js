...
function BuyerViewSoldListingUI(props) {
  ...
  useEffect(() => {
    if (isClick) {
      axios
        .post(
          `http://127.0.0.1:5000/BuyerPostSoldFavorite/${propertyName}`,
          {
            username: localStorage.getItem("username"),
          },
          {
      ...
      <>
        ...
        >
          ...
          <div className="relative">
            ...
            {/* If property sold */}
            {isSold && (
              <div className="absolute inset-0 bg-black bg-opacity-50 rounded-lg flex items-center justify-center text-white text-4xl font-bold">
                Sold
              </div>
            )}
          </div>
