...
function BuyerViewFavoriteListingUI(props) {
  ...
  useEffect(() => {
    ...
    if (load) {
      axios
        .post(
          `http://127.0.0.1:5000/BuyerViewFavoriteListing/${propertyName}`,
          {
  ...
  useEffect(() => {
    if (isClick) {
      axios
        .post(
          `http://127.0.0.1:5000/BuyerPostNewFavorite/${propertyName}`,
          {
            username: localStorage.getItem("username"),
          },
          {
  ...
  function displayFavoriteListingUI() {
    return (
      <>
        <UserContextProvider>
          <BuyerHeader />
  ...
  return displayFavoriteListingUI();
}

export default BuyerViewFavoriteListingUI;
